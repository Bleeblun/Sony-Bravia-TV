...
static u32 fgOpenStBeforeSuspend=FALSE;//to record open status before enter suspend for recovery after resume
...
/*
    if(&(this_device->dev) != dev)
    {
        STAR_MSG(STAR_ERR, "Not ethernet eth0 device\n");;
        return 0;
    }
...
*/    
    ...
    if(netdev!=NULL)
    {
        ...
        StarSetBit(eth_base + 0x300, 1 << 16);
        star_stop(netdev);
...
*/
	...
	if(netdev!=NULL)
	{
		...
		#if defined (CONFIG_ARCH_MT5398) || defined (CONFIG_ARCH_MT5399) || defined (CONFIG_ARCH_MT5880) || defined (CONFIG_ARCH_MT5881) || defined(CONFIG_ARCH_MT5890) || defined(CONFIG_ARCH_MT5882) || defined(CONFIG_ARCH_MT5883)|| defined(CONFIG_ARCH_MT5891) 
        StarClearBit(eth_base + 0x300, 1 << 16);
        STAR_MSG(STAR_DBG, "0x300 register is %x\n", StarGetReg(eth_base + 0x300));
        ...
        STAR_MSG(STAR_DBG, "star_resume 0x32308 value(0x%x)\n", StarGetReg(eth_base+0x308));
