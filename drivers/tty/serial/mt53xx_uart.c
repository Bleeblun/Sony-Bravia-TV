...
#if 1 /* Tx lost interrupt */

#define  TX_TIMER_INTERVAL   (5*HZ)
static void _mt53xx_uart_tx_timer(unsigned long data);
static DEFINE_TIMER(uart_tx_timer, _mt53xx_uart_tx_timer, 0, 0);

static void _mt53xx_uart_tx_timer(unsigned long data)
{
    struct mt53xx_uart_port* mport0 = &_mt53xx_uart_ports[0];
    struct uart_port* port;

    port = &mport0->port;
    uart_write_wakeup(port);
    mod_timer(&uart_tx_timer, jiffies + TX_TIMER_INTERVAL);
}
#endif /* Tx lost interrupt */
...
static void _mt53xx_u4_int_enable(int enable)
{
    ...
    {
        unsigned int u4Reg;
        u4Reg = __raw_readl(PDWNC_VIRT + 0x44);// bit 5 is HT interrupt enable bit
        u4Reg |= 0x20;
        __raw_writel(u4Reg, PDWNC_VIRT + 0x44);
                                       ...
                                       unsigned int count)
{
...
#endif
    {
        ...
        if (co->index == 0)
        {
            ...
            UART_WRITE32(u4Tmp, UART_INT_EN);
            //UART_WRITE32(U0_INTALL | U0_TBUF, UART_INT_EN);//may be clear dma interrupt register 
        }
        if (co->index == 1)
        {
