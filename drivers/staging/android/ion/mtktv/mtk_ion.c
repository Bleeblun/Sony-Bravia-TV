...
#endif
#define ION_SYS_CACHE_SYNC 0x1

typedef struct ion_sys_data_
{
  unsigned int type;
...
static long mtk_ion_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd)
	{
		...
		{
			ion_sys_data data; /* Current var */

            if (copy_from_user((void *)&data, pvArg, sizeof(ion_sys_data)))
            {
                printk(KERN_WARNING "mtk_ion_ioctl error: Copy from user error!\n");
                return -EFAULT;
            }
			//printk("test mtk_ion_ioctl 00 0x%llx, size=0x%x\n", data.addr, data.size);
			dmac_unmap_area((void *)data.addr, data.size, DMA_FROM_DEVICE);
			break;
		}
		default:
			break;
...
int tegra_ion_probe(struct platform_device *pdev)
{
	...
	idev = ion_device_create(mtk_ion_ioctl);
