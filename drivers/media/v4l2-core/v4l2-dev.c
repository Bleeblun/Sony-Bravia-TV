		...
		int warn_if_nr_in_use, struct module *owner)
{
	...
	printk("Start to set the Camera node origianl node = %d cnt = %d.\n", nr, get_camera_1st_node());
	if(get_camera_1st_node())
	{	
		// first video inode = video0, the 2nd is video1;
		nr = 2- get_camera_1st_node();
		printk("Set Video %d for the Camera node.\n", nr);
		clean_camera_1st_node();
	}
	if (nr == minor_cnt)
