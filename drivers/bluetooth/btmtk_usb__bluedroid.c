...
#include "btmtk_usb__bluedroid.h"
#include <linux/skbuff.h>
...
static int btmtk_usb_send_hci_suspend_cmd(struct usb_device *udev)
{
	...
	printk("new btmtk_usb_send_hci_suspend_cmd : OK\n");
	...
	printk("ret:0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x\n",
		...
		if (buf[0] == 0xE4 &&
			...
			printk("Check wmt suspend event result : OK\n");
		} else {
			printk("Check wmt suspend event result : NG\n");
		}
#endif
...
static int btmtk_usb_send_assert_cmd(struct usb_device *udev)
{
	...
	printk("btmtk_usb_send_assert_cmd : OK\n");
	return 0;
}
...
ssize_t BT_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (count > 0) {
		...
		if (copy_from_user(&o_buf[0], &buf[0], copy_size)) {
			retval = -EFAULT;
			printk("btmtk:BT_write: copy data from user fail\n");
			goto OUT;
		}
		...
		if (o_buf[0] == 0x01) {
			...
			printk("btmtk:BT_write:this is unknown bt data:0x%02x\n", o_buf[0]);
		}
	} else {
		retval = -EFAULT;
		printk("btmtk:BT_write:target packet length:%zu is not allowed, retval = %d.\n", count, retval);
	}
