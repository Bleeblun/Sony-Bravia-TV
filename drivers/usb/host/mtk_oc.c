...
static int proc_oc_message(void __user *arg)
{
    ...
    *buf = 0;
    ...
    if (id >= MUCUST_GetControllerNum() && (id != 0xFF))
    {
        INFO("[USB]argument error \n");
        return 1;
    }
...
static int MGC_USBOCThreadMain(void * pvArg)
{
   ...
   //      should NOT set pinmux here because pins might depends on customer and pcba
   ...
   do{
	 ...
	 for(bPortNum=0;bPortNum<MUC_NUM_MAX_CONTROLLER;bPortNum++)
	 {
	    ...
	    if(ssusb_adb_flag && (bPortNum == MUSB_GADGET_PORT_NUM))
	       continue;
#endif
	    if((MUC_aPwrGpio[bPortNum] != -1)&&(MUC_aOCGpio[bPortNum] != -1))
	    {
	       do
	       {
	 ...
	 if (fgOCStatus != u1MUSBOCStatus)
	 {
	    ...
	    {
	       printk("[USBOC]: Turn off Vbus GPIO off\n");
	 ...
	 else
	 {
	    ...
	    if (fgOCStatus)
	    {
	       ...
	       {
		  printk("[USBOC]2: Turn off Vbus GPIO off\n");
		  u1MUSBVbusEanble = FALSE;
#ifdef CONFIG_MT53XX_NATIVE_GPIO
		  for(bPortNum=0;bPortNum<MUC_NUM_MAX_CONTROLLER;bPortNum++)
		  {
