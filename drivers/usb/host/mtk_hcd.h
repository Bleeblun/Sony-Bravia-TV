 ...
 *  Other files: 
 *  1. \sound\usb\Endpoint.c\snd_usb_init_substream_urbs()
 *  2. \sound\usb\Endpoint.c\prepare_capture_urb()
 *  3. \drivers\usb\core\usb_submit_urb()
 */
//#define MGC_RX_LENGTH_HANDLE
#ifdef MGC_RX_LENGTH_HANDLE
#define EXTEND_BUFFER_TIMES     2
// if the ep's MaxPkt > 64, the GPD's "Allow Data Buffer Length" will be set 5 times of the MaxPkt.
// e.g. audio iso ep = 98, the GPD's data length will be set as 98 x 5
// e.g. video iso ep = 3k, the GPD's data length will be set as 3k x 5.
#define ISO_MIN_MAX_PACKET_SZ      64
#endif

/*20141216 Leon WIFI sometimes suspend/resume times isn't match, it cuases wifi resume flow abnormal.
 ...
 *wifi dongle if system is resume from not suspend state..
 * linux-3.10/arch/arm/mach-mt5xx/sleep_coreoff.c
 * linux-3.10/kernel/time/alarmtimer.c  : only for test return -BUSY forever.
 * linux-3.10/drivers/usb/core/hub.c:   disable device disconnect action.
 */
#define MUSB_SUPPORT_SKIP_SUSPEND
