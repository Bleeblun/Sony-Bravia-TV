...
int mgc_adbenable= 0;
module_param(mgc_adbenable, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(mgc_adbenable,"Host Debug level");
	...
	void *dummy)
{
	...
	if (event == PM_SUSPEND_PREPARE) {
		...
		if (Is_WSW_Insert) bWaitWSW=1;
		else bWaitWSW=0;
		printk(KERN_ERR "WSW is %d  (1:on, else :remove)\n",bWaitWSW);
		#if 0 //follow proposal_e
		for(bPortNum=0; bPortNum< MUC_NUM_MAX_CONTROLLER; bPortNum++)
		{
			#ifdef MUSB_GADGET_PORT_NUM
			if(ssusb_adb_flag && (bPortNum == MUSB_GADGET_PORT_NUM))
			{
				continue;
			}
			#endif
		...
		//wait for finish the usb device disconnect related action. 
		//WSW need 3000 ms to complete and release wake lock 
		if (bWaitWSW)
			msleep(3000); 
		else
			msleep(2000); 
...
static void usb_events_timer_func (unsigned long _pThis)
{
	...
	{
		INFO("[USB%d]USB_EVENTS_BUS_RESUME_OFF\n", pThis->bPortNum);
...
static MGC_LinuxLocalEnd *MGC_FindEnd(MGC_LinuxCd *pThis, struct urb *pUrb)
{
	...
	if((pDescriptor->idVendor==WSW_VID)&&(pDescriptor->idProduct==WSW_PID))
	{
		Is_WSW_Insert=1;
		INFO("WSW device insert , set Is_WSW_Insert = 1.\n");
	}

	if(MGC_WIFIHotbootSupported(pUrb))
	{
		INFO("is_musb_suspend = %d.\n", is_musb_suspend);
		if(mtk_usb_suspend())
		{
			is_musb_suspend=0;
		}
...
#if 0
	if((mgc_resumtcnt < mgc_printnum) && usb_pipebulk(pUrb->pipe)&&(usb_pipein(pUrb->pipe)== 0))
	{
	INFO("[USB %d]pUrb=0x%p, bChannel=%d, bReEnd=%d, proto=%s\n", mgc_resumtcnt, pUrb, bBit, usb_pipeendpoint(pUrb->pipe), MGC_DecodeUrbProtocol(pUrb));
	}
#endif
            break;
        }
...
static void MGC_KickCmdQ(MGC_LinuxCd *pThis, MGC_LinuxLocalEnd *pEnd, struct urb *pUrb)
{
    ...
    if (usb_pipeisoc(nPipe))
    {
        ...
        for(i=0; i<pUrb->number_of_packets; i++)
        {
			...
			//DBG(-9, "[%d]ISO dwLength = 0x%x, mxPakt = 0x%x.\n", i, dwLength, pEnd->wMaxPacketSize);
			if(dwLength > ISO_MIN_MAX_PACKET_SZ)
			{
				dwLength = pUrb->iso_frame_desc[i].length * EXTEND_BUFFER_TIMES;
			}
			#endif
        ...
        */
        ...
        else
        {
			...
			if(MGC_WIFIHotbootSupported(pUrb))
			{
				...
				#if 0
				if((mgc_resumtcnt < mgc_printnum))
				{
					INFO("[%d]URB new = %d.\n", mgc_resumtcnt, bInterval);
				}
				#endif
			}
			#endif
...
static int __init MUC_init(void)
{
	...
	mgc_adbenable = ssusb_adb_flag;
	INFO("mgc_adbenable = %d\n", mgc_adbenable);
    #ifdef CONFIG_ARCH_MT85XX
    ...
    for (i = 0; i < nCount; i++)
    {
		...
		if((MUC_aPortUsing[i] == 1) 
			#ifdef MUSB_GADGET_PORT_NUM
			&& (!(ssusb_adb_flag && (i == MUSB_GADGET_PORT_NUM)))
			#endif
			)
		#endif
	    {
