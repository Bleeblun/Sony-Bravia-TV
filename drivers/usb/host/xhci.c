...
unsigned int recovery_enable = 1;
...
static void compliance_mode_recovery(unsigned long arg)
{
    ...
    if (xlevel == 1) {
	xhci_info(xhci, "[%s] usb_HDDREGFlag = 0x%x, port status = 0x%x\n", __func__, usb_HDDREGFlag, xhci_readl(xhci, xhci->usb3_ports[0]) & PORT_PLS_MASK);
    }
    ...
    timer_delay = COMP_MODE_RCVRY_MSECS;
