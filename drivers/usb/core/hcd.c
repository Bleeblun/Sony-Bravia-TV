...
void usb_hcd_poll_rh_status(struct usb_hcd *hcd)
{
	...
	if (unlikely(!hcd->rh_pollable)){
		mtk_usb_connect_dbg("[USBCONNECT] !!! %s: rh_pollable is NULL.\n", __func__);
		return;
	}
	if (!hcd->uses_new_polling && !hcd->status_urb) {
		mtk_usb_connect_dbg("[USBCONNECT]!!! %s: hcd->status_urb is NULL.\n", __func__);
		return;
	}

	length = hcd->driver->hub_status_data(hcd, buffer);
	mtk_usb_connect_dbg("[USBCONNECT] [%s] port is %s.\n", __func__, length? "change":"not change");
	if (length > 0) {
