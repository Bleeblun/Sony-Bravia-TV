...
#else //for async mmc request support
int msdc_partread(int partno, uint64_t offset, uint64_t size, void *pvmem)
{
	struct file *filp;
	char buff[30] = {0};
	...
	//printk(KERN_ERR "msdc_partread(%s) oft 0x%llx size ox%llx",buff,offset,size);
    BUG_ON(msdc_check_partition());
	...
	else
	{
		...
		if (seekpos != offset)
		{
			printk(KERN_INFO "[%s]%s seek failed,seekpos:0x%08X \n", __func__, buff, (u32)offset);
		...
		if (rd_length != size)
		{
			printk(KERN_ERR "[%s]%s R/W length not match requested,request length:0x%08X, return length:0x%08X\n", __func__, buff, 
...
int msdc_partwrite(int partno, uint64_t offset, uint64_t size, void *pvmem)
{
   ...
   // printk(KERN_ERR "msdc_partwrite(%s) oft 0x%llx size ox%llx",buff,offset,size);
