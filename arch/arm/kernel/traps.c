...
void dump_kernel_stack_on_cpu(int cpu, struct task_struct *tsk)
{
	unsigned long ti = (unsigned long)task_stack_page(tsk);

	printk("current task on cpu %u\n", cpu);
	printk("pid = %d tgid = %d comm=\"%s\"\n",
		tsk->pid, tsk->tgid, tsk->comm);
	dump_mem("", "thread_info/kernel stack: ", ti, ti + THREAD_SIZE);
}

void dump_DBGPCSR_on_all_cpus(void)
{
#if defined(CONFIG_ARCH_MT5890)
   extern void __iomem *pDBGPCSRIoMap[NR_CPUS];
   int cpu;
   u32 dbgpcsr;
   int i, n_sample = 10;
   ...
   for(i=0; i<n_sample; i++) {
       for_each_online_cpu(cpu) {
           if (pDBGPCSRIoMap[cpu] != NULL) {
               dbgpcsr = ioread32(pDBGPCSRIoMap[cpu]);
               printk("DBGPCSR[%d] : 0x%x  %pF\n", cpu, dbgpcsr, (void*)dbgpcsr);
           }
       }
       touch_nmi_watchdog();
       local_touch_nmi_watchdog();
