...
#define MCU_BASE					(IO_VIRT + 0x78000)
...
#define MCU_READ32(off)             IO_READ32(MCU_BASE, off)
#define MCU_WRITE32(off, val)       IO_WRITE32(MCU_BASE, (off), (val))
...
u64 mt53xx_read_local_timer(void)
{
    ...
    val = MCU_READ32(XGPT_CTRL); // high_32_bit
    cnt = (u64)(((u64)val) << 32);
    MCU_WRITE32(XGPT_IDX, 0x8);
    val = MCU_READ32(XGPT_CTRL); // low_32_bit
    cnt = cnt | val;
    return cnt;
    */
